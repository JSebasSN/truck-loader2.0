...
function LoadPicker({ open, onClose, position, loadNames, onPick }) {
  ...
  return <Modal open={open} onClose={onClose} title={`Cargar Posición ${position}`} sub="Seleccionar destino"
    footer={manual.trim() ? <button className="btn btn-a btn-bl" onClick={pickManual}>Guardar "{manual.trim().toUpperCase()}"</button> : null}>
    {loadNames.map(ln => <button key={ln} className="lo" onClick={() => onPick(ln)}>{ln}</button>)}
    <div style={{ textAlign: "center", color: "var(--t3)", fontSize: 12, margin: "12px 0 8px" }}>O escribe manualmente</div>
  ...
  return <Ctx.Provider value={{ user, trucks, users, assigns, history }}>
    <div className="shell">
      <aside className="sb"><div className="sb-logo"><span>TruckLoader</span></div><nav className="sb-nav">{nav.map(n => <button key={n.id} className={`si ${tab === n.id ? "on" : ""}`} onClick={() => setTab(n.id)}><n.ic />{n.l}</button>)}</nav>
        <div className="sb-ft"><div className="sb-u"><div className="av" style={{ background: avBg(user.name), width: 32, height: 32, fontSize: 13 }}>{user.name[0]}</div><div><div className="sb-un">{user.name}</div><div className="sb-ur">{adm ? "Administrador" : "Carretillero"}</div></div></div><button className="slo" onClick={() => signOut(auth)}><I.Logout /> Cerrar Sesión</button></div></aside>
